...
const std::unordered_map<
    char, std::function<int(std::vector<mod_type<int>>,
                            std::vector<mod_type<int>>, int, int, int)>>
    color_to_function_map = {
        {'r',
         [](const std::vector<mod_type<int>>& a,
            const std::vector<mod_type<int>>& b, const int& x, const int& s,
            const int& d) -> int {
...
int one_color_flash(const std::string& flash,
                    ...
                    const std::vector<mod_type<int>>& charlie, const int& step,
                    const int& delta) {
...
int two_color_flash(const std::string& flash,
    ...
    if (color_mix_value == 2) {
        ...
        answer +=
            (color_to_function_map.find(absent_color)
                 ->second(alpha, bravo, int(charlie[step - 1]), step, delta) +
             (color_to_function_map.find(absent_color)
                  ->second(alpha, bravo, int(bravo[step - 1]), step, delta)) +
             (color_to_function_map.find(absent_color)
                  ->second(alpha, bravo, int(alpha[step - 1]), step, delta)));
...
int three_color_flash(const std::string& flash,
    ...
    if (color_mix_value == 3) {
        answer +=
            int((charlie[step - 1] + ((charlie[step - 1] % 3) * charlie[0])) -
        ...
        answer +=
            int((charlie[step - 1] + ((charlie[0] % 3) * charlie[step - 1])) -
