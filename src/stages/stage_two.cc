...
const std::unordered_map<
    ...
    color_to_function_map = {
        {'r',
         [](const std::vector<mod_type<int>>& a, const int& x, const int& s,
            const int& d) -> int {
...
int one_color_flash(const std::string& flash,
                    ...
                    const std::vector<mod_type<int>>& bravo, const int& step,
                    const int& delta) {
    ...
    if (color_mix_value == 2 || color_mix_value == 3) {
        ...
        const int first =
            color_to_function_map.find(absent_color)
                ->second(alpha, int(alpha[step - 1]), step, delta);
        const int second =
            color_to_function_map.find(absent_color)
                ->second(alpha, int(bravo[step - 1]), step, delta);
...
int three_color_flash(const std::string& flash,
    ...
    if (color_mix_value == 3) {
        answer += int(bravo[step - 1] + ((bravo[step - 1] % 4) * bravo[0]) -
                      alpha[3]);
        ...
        answer += int(bravo[step - 1] + std::accumulate(color_values.begin(),
                                                        color_values.end(), 0));
        ...
        answer += int(bravo[step - 1] + ((bravo[0] % 4) * bravo[step - 1]) -
                      alpha[3]);
    }
